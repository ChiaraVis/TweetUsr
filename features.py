...
def extract_features(statuses):
  ...
  # mmv of hour
  x.extend(mean_median_variance(statuses,lambda x:parse(x['created_at']).hour))
  # mmv of day
  x.extend(mean_median_variance(statuses,lambda x:parse(x['created_at']).weekday()))
  # during working hours
  x.append(percentage(statuses,lambda x:parse(x['created_at']).weekday()<6 and parse(x['created_at']).hour>=7 and parse(x['created_at']).hour<=18))
  # on weekend
  x.append(percentage(statuses,lambda x:parse(x['created_at']).weekday()>5))
  # source twitter web client
  x.append(percentage(statuses,lambda x:u'Twitter Web Client' in x['source']))
  # truncated
  ...
  x.append(user_meta(statuses,lambda x:float(x[-1]['user']['statuses_count'])/(datetime.now()-parse(x[-1]['user']['created_at']).replace(tzinfo=None)).days))
  # followers count
...
if __name__=='__main__':
  ...
  for user_num,user in enumerate(users):
    ...
    if len(files)<100:
      continue
    for file in files:
      statuses.append(json.load(open('/home/nikola/tools/tweetpub/statuses/'+user+'/'+file)))
    print 'loaded',
    ### text for bow ###
